#!/usr/bin/env python3
"""Legacy rich.Layout TUI dashboard for daemon_base_v1.

This is the pre-Textual implementation, restored as the default TUI in
response to the Textual rewrite not being to user preference. Reads
daemon_state/state.json (refreshed by the daemon every 5s) and
daemon_state/daemon.log + events.jsonl (streamed live), renders a
single full-screen layout that refreshes at ~2 Hz.

Run standalone:
    conda activate polymarket-env
    python3 tui/python/dashboard_legacy.py

Or via the launcher (this is the default):
